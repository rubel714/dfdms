...
function getDataList($data){
	...
	try{
		...
		$query = "SELECT COUNT(f.`PGId`) AS TotalPG FROM `t_pg` f where f.IsActive=1 and f.StatusId=5;";
		$resultdata = $dbh->query($query);
		foreach ($resultdata as $key => $row) {
			$TotalPG = $row["TotalPG"];
		}

		$query = "SELECT COUNT(f.`PGId`) AS TotalFarmer FROM `t_farmer` f where f.IsActive=1 and f.StatusId=5;";
		$resultdata = $dbh->query($query);
		foreach ($resultdata as $key => $row) {
...
function getGenderwisePGMemberDataList($data){
	...
	try{
		...
		INNER JOIN `t_gender` g ON f.`Gender` = g.GenderId
		 where f.IsActive=1 and f.StatusId=5
