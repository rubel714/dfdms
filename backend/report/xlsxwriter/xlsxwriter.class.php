...
class XLSXWriter
{
	...
	protected function initializeSheet($sheet_name, $col_widths=array(), $auto_filter=false, $freeze_rows=false, $freeze_columns=false )
	{
		...
		// $sheet->file_writer->write(  '<col  max="1024" min="1"  customWidth="10" width="15"/>');
		// $sheet->file_writer->write(  '<col  max="1024" min="1" width="AutoFit"/>');
		// $sheet->file_writer->write(  '<col  max="1024" min="1" width="10" BestFit="true" CustomWidth = "true"/>');
		// $sheet->file_writer->write(  '<col collapsed="false" hidden="false" bestFit="true" max="1024" min="'.($i+1).'" style="0" customWidth = "false"  width="12" />');
		// $sheet->file_writer->write(  '<col collapsed="false" hidden="false" max="1024" min="'.($i+1).'" style="0" customWidth="false" width="11.5"/>');
		$sheet->file_writer->write(  '</cols>');
	...
	public function writeSheetHeader($sheet_name, array $header_types, $col_options = null)
	{
		...
		$styleMain = $col_options;
		
		$style = &$col_options;

		$report_headers = isset($col_options['report_headers']) ? (array)$col_options['report_headers'] : array();
		...
		if (!$suppress_row)
		{
			/////////////////add//Rubel 12_05_2019//For report multiple header/////////////////////////
			$report_header_font_size = 14;
			// echo "<pre>";
			// print_r($report_headers);
			// echo "hello";
			foreach ($report_headers as $r => $v) {
				
				$style = &$col_options;
				// $style["font-size"] = $report_header_font_size - $r;
				$style["font-size"] = $report_header_font_size - ($r > 3 ? ($r-3) : 0); // first 4 line font size same=14. after that decrease 1,2,3....
				$style["fill"] = '#d9e1ec';
				$style["halign"] = 'center';
				$style["border"] = '';
				$style["border-style"] = '';
				...
				$cell_style_idx = empty($style) ? $sheet->columns[0]['default_cell_style'] : $this->addCellStyle( 'GENERAL', json_encode(isset($style[0]) ? $style[0] : $style) );
				// $this->writeCell($sheet->file_writer, 0, $c, $v, $number_format_type='n_string', $cell_style_idx);
				$this->writeCell($sheet->file_writer, $sheet->row_count, 0, $v, $number_format_type='n_string', $cell_style_idx);
				$sheet->file_writer->write('</row>');
				$sheet->row_count++;
			}
			// exit;
			// $sheet->row_count++;
			$style = &$styleMain; //main style for table header
			//////////////////////////////////////////////
			// echo "<pre>";
			// print_r($header_types);
			// exit;
			$header_row = array_keys($header_types);
			$sheet->file_writer->write('<row collapsed="false" customFormat="false" customHeight="false" hidden="false" ht="12.1" outlineLevel="0" r="' . ($sheet->row_count+1) . '">');
			foreach ($header_row as $c => $v) {
				
				/*********Add**Rubel** 2019_05_12****************/
				$style["halign"] = $this->getColumnAlign($header_types[$v]);
				...
				// $this->writeCell($sheet->file_writer, 0, $c, $v, $number_format_type='n_string', $cell_style_idx);
				$this->writeCell($sheet->file_writer, $sheet->row_count, $c, $v, $number_format_type='n_string', $cell_style_idx); ////////////Change///////Rubel 12_05_2019//For report multiple header/////////////////////////
			}
			$sheet->file_writer->write('</row>');
			$sheet->row_count++;
		}
	...
	/*********Add**Rubel** 2019_05_12****************/
	public function getColumnAlign($header_types)
	{
		$align = "left";
		if($header_types == "0")
			$align = "center";
		else if($header_types == "integer" || (strpos($header_types,"#") !== false))
			$align = "right";
		else
			$align = "left";
	...
	//added this function by rubel 04/03/2023
	public function writeCellextra($sheet_name,$row_number, $column_number, $value, $style){
		$sheet = &$this->sheets[$sheet_name];
		...
		$sheet->file_writer->write('<row collapsed="false" customFormat="false" customHeight="true" hidden="false" ht="'.$ht.'" outlineLevel="0" r="' . ($row_number+1) . '">');
		$cell_style_idx = empty($style) ? $sheet->columns[0]['default_cell_style'] : $this->addCellStyle( 'GENERAL', json_encode(isset($style[0]) ? $style[0] : $style) );
		$this->writeCell($sheet->file_writer, $row_number, $column_number, $value, $number_format_type='n_string', $cell_style_idx);
		$sheet->file_writer->write('</row>');
