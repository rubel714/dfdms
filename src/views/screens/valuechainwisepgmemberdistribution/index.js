...
import ValueChainWisePGMemberDistribution from "./ValueChainWisePGMemberDistribution";

const Index = (props) => {
  const { path } = useRouteMatch();
  const menukey = "pgentryform"; // this is in t_menu table
      ...
      <div >
        ...
        <Switch>
          <Route
            path={`${path}/`}
            render={(props) => (
              <ValueChainWisePGMemberDistribution {...props} openNoticeModal={openNoticeModal} />
            )}
          ></Route>
        </Switch>
        <Notification
