...
import HouseholdLivestockSurveyForUser from "./HouseholdLivestockSurveyForUser";

const Index = (props) => {
  const { path } = useRouteMatch();
  const menukey = "householdlivestocksurveyforuser"; // this is in t_menu table
      ...
      <div >
        ...
        <Switch>
          <Route
            path={`${path}/`}
            render={(props) => (
              <HouseholdLivestockSurveyForUser {...props} openNoticeModal={openNoticeModal} />
            )}
          ></Route>
        </Switch>
        <Notification
