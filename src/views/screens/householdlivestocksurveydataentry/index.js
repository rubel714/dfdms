...
import HouseholdLivestockSurveyDataEntry from "./HouseholdLivestockSurveyDataEntry";

const Index = (props) => {
  const { path } = useRouteMatch();
  const menukey = "householdlivestocksurveydataentry"; // this is in t_menu table
      ...
      <div >
        ...
        <Switch>
          <Route
            path={`${path}/`}
            render={(props) => (
              <HouseholdLivestockSurveyDataEntry {...props} openNoticeModal={openNoticeModal} />
            )}
          ></Route>
        </Switch>
        <Notification
