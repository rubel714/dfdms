...
import FarmersDataCollection from "./FarmersDataCollection";
import "../../../assets/css/pgdatacollection.css";

// receive.css
const Index = (props) => {
  const { path } = useRouteMatch();
  const menukey = "farmersdatacollection"; // this is in t_menu table
      ...
      <div >
        ...
        <Switch>
          <Route
            path={`${path}/`}
            render={(props) => (
              <FarmersDataCollection {...props} openNoticeModal={openNoticeModal} />
            )}
          ></Route>
        </Switch>
        <Notification
