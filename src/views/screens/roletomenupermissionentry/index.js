...
import RoleToMenuPermissionEntry from "./RoleToMenuPermissionEntry";
import "../../../assets/css/pgdatacollection.css";

// receive.css
const Index = (props) => {
  const { path } = useRouteMatch();
  const menukey = "roletomenupermissionentry"; // this is in t_menu table
      ...
      <div >
        ...
        <Switch>
          <Route
            path={`${path}/`}
            render={(props) => (
              <RoleToMenuPermissionEntry {...props} openNoticeModal={openNoticeModal} />
            )}
          ></Route>
        </Switch>
        <Notification
