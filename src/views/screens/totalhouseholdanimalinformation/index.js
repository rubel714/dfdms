...
import TotalHouseholdAnimalInformation from "./TotalHouseholdAnimalInformation";

const Index = (props) => {
  const { path } = useRouteMatch();
  const menukey = "totalhouseholdanimalinformation"; // this is in t_menu table
      ...
      <div >
        ...
        <Switch>
          <Route
            path={`${path}/`}
            render={(props) => (
              <TotalHouseholdAnimalInformation {...props} openNoticeModal={openNoticeModal} />
            )}
          ></Route>
        </Switch>
        <Notification
